...
| **T7 (dialog/prompt spoofing)** | The per-host grant prompt is a prime spoofing target → rendered in the badged anti-spoof frame (spec §5.5), rate-limited, host origin shown verbatim. **Not yet delivered:** punycode decoding for display and a mixed-script/homograph warning (`TODO(8B.5-homograph)`) — today the origin is shown in its punycode (`xn--`) form, which keeps a homograph visible rather than hidden. |
