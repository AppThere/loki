...
//!
//! # The arithmetic, and the one thing that makes it wrong
//!
//! A content point `d` sits at viewport offset `a` when the scroll is `s` and the
//! zoom is `z`: `a = d·z − s`. Holding `a` fixed across a zoom change gives
//!
//! ```text
//! s₁ = (s₀ + a)·(z₁/z₀) − a
//! ```
//!
//! The thing that makes it wrong is using the **effective** zoom on one side and
//! the **requested** zoom on the other. Spec 08 T5.4 stores those separately and
//! they differ whenever the capability cap bites, so a caller that mixes them
//! anchors against a ratio no rendering ever used. Both arguments here are the
//! zoom the *content was and will be painted at* — see
//! [`anchored_scroll`]'s note.
...
pub fn anchored_scroll(
    ...
    let x = (scroll.0 + ax).mul_add(ratio, -ax).max(0.0);
    let y = (scroll.1 + ay).mul_add(ratio, -ay).max(0.0);
    (x, y)
}
