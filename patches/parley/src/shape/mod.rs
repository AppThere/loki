...
pub(crate) fn shape_text<'a, B: Brush>(
        ...
        text.char_indices().enumerate().zip(infos)
    {
        ...
        if break_run && !text_range.is_empty() {
            ...
            // PATCH(loki): refresh the spacing on a run break.
            //
            // Upstream 0.6.0 refreshes every other field here and leaves these
            // two, so a shape item keeps `styles[0]`'s spacing for the whole
            // layout: a run that asks for letter-spacing gets it only if it is
            // the *first* run, and then every other run gets it too. Measured
            // through Blitz (ADR-0017 §5.6): a tracked span first widened all 36
            // characters of the paragraph, and a tracked span anywhere else
            // widened none.
            //
            // Fixed upstream in parley 0.10.0, which this workspace's own
            // `loki-layout` already uses — so the canvas path was correct and
            // the DOM path was not, for no reason visible in either. Remove this
            // patch when the Blitz stack moves off parley 0.6.
