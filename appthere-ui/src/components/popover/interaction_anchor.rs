...
//! second.
//!
//! # There is no frame source; the driver is event-driven (r72, D-15)
//!
//! The claim that once ended this section — "the frame source already exists:
//! the scroll animator's worker-thread tick" — is **retracted**. It is an
//! animation clock, not a frame source. The full reasoning, the counter's change
//! of unit, and the write-idempotence obligation that comes with it live in
//! [`counter`], because all three are about the one hazard the comparison itself
//! can create.
//!
