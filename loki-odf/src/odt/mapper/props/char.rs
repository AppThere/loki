...
//! Character property mapper: [`OdfTextProps`] → [`CharProps`].
...
pub(crate) fn map_underline_style(s: &str) -> Option<UnderlineStyle> {
    match s {
        "none" => None,
...
pub(crate) fn map_strikethrough_style(s: &str) -> Option<StrikethroughStyle> {
    match s {
        "none" => None,
...
pub(crate) fn map_text_position(s: &str) -> Option<VerticalAlign> {
