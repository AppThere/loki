...
impl DioxusDocument {
    ...
    /// PATCH(loki): drain the elements that registered an `onmounted` listener
    /// since the last call, resolved to `(ElementId, blitz NodeId)` pairs.  The
    /// embedder (`dioxus-native`) dispatches the `mounted` event for each, since
    /// only it can build the [`MountedBackend`](crate::MountedBackend) transport
    /// that needs the event-loop proxy.
    pub fn take_pending_mounted(&mut self) -> Vec<(ElementId, usize)> {
        std::mem::take(&mut self.vdom_state.pending_mounted)
            .into_iter()
            .filter_map(|eid| {
                self.vdom_state
                    .try_element_to_node_id(eid)
                    .map(|nid| (eid, nid))
            })
            .collect()
    }
