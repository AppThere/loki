...
| **Editor integration** | Pending | Live checking of visible paragraphs, mapping `Misspelling` byte ranges onto layout to emit `Spelling` decorations, and a right-click suggestions menu (no `oncontextmenu` precedent in the editor yet) are the remaining UI pass — they require runtime GUI verification. The app still needs to provide a `DictionaryFetcher` (its HTTPS client) and choose the store's data dir. |
...
Coverage of the engine + dictionary layers is 39 unit/integration tests (incl. a real load of the bundled `en` dictionary).
