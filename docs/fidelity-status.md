...
| **ODT export** | Yes | — | Yes | `loki-odf`'s `OdtExport` writes a full ODT package (`content.xml` / `styles.xml` / `meta.xml`): paragraphs, headings, styled paragraphs, lists, tables, inline formatting (bold/italic/underline/strike/super/sub/small-caps/links/footnotes), the named style catalog, page geometry, and Dublin Core metadata. Editing an opened `.odt` and saving round-trips to ODT. Not yet emitted: images, fields, list-numbering styles, and the OTT template content type. |
