...
//! Vertical-merge span resolution for the DOCX table mapper (split from
//! `table.rs` for the 300-line ceiling): the two-pass `w:vMerge` algorithm
//! (§17.4.84) that assigns `row_span` to restart cells and marks continuation
//! cells for omission. Re-imported by `table.rs` (and exercised by its tests).
...
pub(super) fn compute_v_merge_spans(
    rows: &[DocxTableRow],
