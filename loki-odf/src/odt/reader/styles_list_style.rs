...
//! `text:list-style` parsing, split out of `styles.rs` for the 300-line
//! ceiling. `parse_list_style` is re-exported from `styles.rs` (called by
//! `read_stylesheet`); it reaches the per-level helpers via `super::list` and
//! the generic skipper via `super::skip_element`.
