...
| **Section Breaks** | Yes | Yes | Yes | Supports multiple sections with different page layouts. **Mixed page sizes/orientations render correctly:** the editor/print painter (`loki-vello` `paint_single_page`/`paint_paginated`) sizes each page's white background and drop shadow from that page's own `page.page_size` (via `page_chrome_size`), not the document-level default — so an A4 or landscape-Letter section no longer leaves a gray streak where a mis-sized background failed to cover the render tile. Tested by `page_chrome_uses_per_page_size`. |
...
| **Line Height** | Yes | Yes | Yes | Supports exact, relative (multipliers), and at-least rules. **`exact` clips**: each line's items are wrapped in a `PositionedItem::ClippedGroup` sized to the fixed line box (`pts` tall, positioned by the same half-leading Parley uses: box top = `baseline − (pts + ascent − descent)/2`), so content taller than the box is cut off as in Word; `atLeast`/`auto` lines grow naturally and are not clipped. Tested by `exact_line_height_clips_each_line`. |
...
| **Nested Tables** | DOCX | Yes | No | A `w:tbl` nested inside a `w:tc` is imported: the DOCX cell model holds ordered block content (`Vec<DocxBodyChild>`), the reader recurses into the inner table, and the mapper produces a `Block::Table` inside the cell's blocks (interleaved with paragraphs in document order). The layout already flows a cell's blocks through `flow_block`/`flow_table`, so the nested table lays out and clips to the cell box with no layout change. Tested by `nested_table_in_cell_maps_to_table_block`. **ODT deferred** (gap): the ODF cell model still holds only paragraphs; the ODT fixture exercises no nesting and the ODF cell mapper lives in the over-ceiling `odt/mapper/document.rs`, which must be split before growing. Not re-exported. |
| **Page Number Format** | DOCX | Yes | No | `w:pgNumType @w:fmt`/`@w:start` flow into `PageLayout.page_number_format`/`page_number_start` and through `FieldContext`; the PAGE field is formatted via the shared list-marker converter (`lowerRoman`→i, ii, iii; `upperRoman`; `lowerLetter`/`upperLetter`), with the section restart offsetting the displayed number from the section's first physical page. NUMPAGES stays decimal. Tested by `substitution_formats_page_number_as_lower_roman`. **ODT deferred** (gap): ODF page-number format lives on the master-page `style:num-format` and is not yet read. Not re-exported. |
