...
| **Comments (annotations)** | Yes | No | Yes | Round-trip in both formats. The commented range is carried as `Inline::Comment` start/end anchors in the content flow; bodies (author, date, plain text) live in `Document.comments`. DOCX uses `w:commentRangeStart`/`w:commentRangeEnd` + a `CommentReference` run, with bodies in `word/comments.xml`; ODF uses inline `office:annotation` (body + `dc:creator`/`dc:date`) / `office:annotation-end`. Tested by `comments_round_trip.rs` (DOCX) and `comments_round_trip` (ODT). Not affecting layout; not yet persisted through the Loro CRDT, and multi-paragraph bodies are flattened to plain text (paragraphs joined by `\n`). |
