...
#[test]
fn no_selection_shows_only_the_three_core_tabs() {
    let tabs = ribbon_tabs(SelectedObject::None);
    assert_eq!(tabs.len(), 3, "Write, Insert, Publish");
    assert!(
...
fn table_selection_appends_a_contextual_tab() {
    ...
    assert_eq!(tabs.len(), 4, "the Table tab is appended");
    // The three core tabs stay non-contextual...
    assert!(tabs[..3].iter().all(|t| !t.is_contextual));
    // ...and the appended Table tab is contextual (renders amber).
    assert!(tabs[3].is_contextual, "the Table tab is contextual");
}
...
fn the_contextual_tab_sits_at_the_reserved_index() {
    ...
    let tabs = ribbon_tabs(SelectedObject::Table);
    assert_eq!(CONTEXTUAL_TAB_INDEX, 3);
