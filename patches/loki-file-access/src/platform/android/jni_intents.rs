...
pub(super) fn take_persistable_uri_permission(uri: &str) -> Result<(), PickerError> {
    ...
    .map_err(|e| {
        // A provider may throw SecurityException ("No persistable permission
        // grants found"). Clear the pending Java exception so callers that
        // tolerate this failure can keep making JNI calls on this thread.
        let _ = env.exception_clear();
        platform_err("takePersistableUriPermission", e)
    })?;
