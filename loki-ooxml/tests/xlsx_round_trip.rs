...
fn test_xlsx_round_trip_basic() {
    ...
    for (sheet_idx, original_sheet) in workbook.sheets.iter().enumerate() {
        ...
        for (coord, original_cell) in &original_sheet.cells {
            let imported_cell = imported_sheet.cells.get(coord)
                .expect(&format!("Cell at {:?} should exist in imported sheet", coord));
            
            assert_eq!(
                imported_cell.value, original_cell.value,
                "Cell value mismatch at sheet {}, coord {:?}", sheet_idx, coord
            );
            assert_eq!(
                imported_cell.formula, original_cell.formula,
                "Cell formula mismatch at sheet {}, coord {:?}", sheet_idx, coord
            );
            ...
            match (&original_cell.style, &imported_cell.style) {
                ...
                (Some(orig_style), Some(imp_style)) => {
                    assert_eq!(imp_style.bold, orig_style.bold, "Bold style mismatch at {:?}", coord);
                    assert_eq!(imp_style.italic, orig_style.italic, "Italic style mismatch at {:?}", coord);
                    assert_eq!(imp_style.underline, orig_style.underline, "Underline style mismatch at {:?}", coord);
                    assert_eq!(imp_style.align, orig_style.align, "Alignment mismatch at {:?}", coord);
                    assert_eq!(imp_style.num_format, orig_style.num_format, "Number format mismatch at {:?}", coord);
                }
                (Some(orig_style), None) => {
                    ...
                    assert_eq!(orig_style, &default_style, "Non-default style was lost at {:?}", coord);
                }
                (None, Some(imp_style)) => {
                    // Check if imported style matches the default style.
                    let default_style = CellStyle::default();
                    assert_eq!(imp_style, &default_style, "Extraneous non-default style imported at {:?}", coord);
                }
            }
        }
    }
