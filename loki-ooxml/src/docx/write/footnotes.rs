...
use super::xml::{write_decl, write_empty, write_end, write_start, wval, NS_W};
...
fn write_notes_xml(
    ...
    write_special_note(&mut w, "continuationSeparator", "0", "w:continuationSeparator");
    ...
    for entry in entries {
        ...
        let _ = write_start(&mut w, "w:r", &[]);
        let _ = crate::docx::write::xml::write_text_elem(&mut w, "w:t", &[("xml:space", "preserve")], " ");
        let _ = write_end(&mut w, "w:r");
...
fn write_special_note<W: std::io::Write>(w: &mut Writer<W>, type_val: &str, id_val: &str, child: &str) {
    let tag = if child.contains("footnote") || child == "w:separator" || child == "w:continuationSeparator" {
        "w:footnote"
    } else {
        "w:endnote"
    };
